...
function seek(&$h,$contained=false){
  foreach($h as $key => &$value){
    if(is_array($value) && array_keys($value) !== range(0, count($value) - 1)){
      $tree=seek($value, $contained || str_contains($key,FILE));
      if (empty($tree)) unset($h[$key]);
      }
    elseif(!str_contains('.'.pathinfo($key,PATHINFO_EXTENSION),EXT) || !($contained || str_contains(pathinfo($key,PATHINFO_FILENAME),FILE))) unset($h[$key]);
    }
    return $h;
}
